...
int main(){
    ...
    Shader shader("../06_transformations/_vert.glsl", "../06_transformations/_frag.glsl");
    ...
    //texture
    int w1, h1, c1;
    unsigned char *data1 = stbi_load("../06_transformations/texture1.png", &w1, &h1, &c1, 0);
    assert(data1);

    int w2, h2, c2;
    unsigned char *data2 = stbi_load("../06_transformations/texture2.png", &w2, &h2, &c2, 0);
    assert(data2);
...
*/
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        trans = glm::rotate(trans, glm::radians(s * 90.0f), glm::vec3(0.0, 0.0, 1.0));//最后一个参数绕 Z 轴
        // trans = glm::scale(trans, glm::vec3(0.5, 0.5, 0.5)); // 最后一个参数说明所有轴都缩放
        std::string transformName = "transform";

        shader.SetMat4(transformName, glm::value_ptr(trans));
